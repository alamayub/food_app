...
import 'package:food_app/repositories/voucher/base_voucher_repository.dart';
...
class VoucherRepository extends BaseVoucherRepository {
  ...
  Stream<List<Voucher>> getVouchers() {
    return _firebaseFirestore.collection('vouchers').snapshots().map((docs) {
      return docs.docs.map((doc) => Voucher.fromSnapshot(doc)).toList();
    });
  }
